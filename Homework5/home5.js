...
let arr = [3, -5, 2, 3, 2, 6, -4, 3, 9, 7, 7, 2]
...
function countPositiveNumbers(array) {
   let positiveNumbers = array.filter(function filter(number){
      return number >0
   })
...
let newArray = [6, 2, 55, 11, 78, 2, 55, 77, 57, 87, 23, 20, 56, 3, 2]
function getDividedByFive (elements) {
   let DividedByFive  = elements.filter(function filtered(number) {
      return number %5 ===0
   })
...
'Функція 4'<br>кількість чисел більших 0<br>${countPositiveNumbers(arr)}<br>
